      ...
      <div className="mx-auto w-full max-w-[1100px] px-4 sm:px-6 lg:px-8">
        ...
        <section className="py-10 sm:py-14">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-2">
            ...
            <section className="rounded-2xl border border-[var(--border)] bg-white p-5">
              <h2 className="text-xl font-semibold">⏱️ Horas extra</h2><p className="mt-1 text-sm text-[var(--muted)]">Base horaria: sueldo mensual ÷ 240. Las horas suplementarias llevan 50% y las extraordinarias 100% de recargo, según corresponda.</p><div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label="Horas al 50%" value={hours50} onChange={setHours50} step="1" /><Field label="Horas al 100%" value={hours100} onChange={setHours100} step="1" /></div><div className="mt-4 grid grid-cols-2 gap-3"><Result label="Valor hora al 50%" value={money.format(values.hourly.supplementary)} /><Result label="Valor hora al 100%" value={money.format(values.hourly.extraordinary)} /></div><div className="mt-3"><Result label="Total estimado de horas extra" value={money.format(values.overtime)} /></div>
            </section>

            <section className="rounded-2xl border border-[var(--border)] bg-white p-5 lg:col-span-2">
              <h2 className="text-xl font-semibold">💰 Utilidades</h2>
              <p className="mt-1 text-sm leading-6 text-[var(--muted)]">Aquí hay una diferencia importante: <strong>tu sueldo no permite saber cuánto te corresponde de utilidades</strong>. El 10% depende de los días de todas las personas que participan y el 5% depende de las cargas familiares de todas ellas. Por eso no te voy a pedir que adivines datos de cientos de compañeros.</p>

              <div className="mt-5 rounded-xl border border-[var(--border)] bg-[var(--background)] p-4">
                <p className="text-sm font-semibold">Lo que sí puedes saber tú</p>
                ...
                <div className="mt-4 rounded-xl border border-[var(--border)] bg-white p-3 text-xs leading-5 text-[var(--muted)]">Estos datos ayudan a determinar tu participación, pero <strong className="text-[var(--foreground)]">no permiten calcular por sí solos el valor en dólares</strong>.</div>
              </div>
              ...
              <div className="mt-5 rounded-xl border border-[var(--border)] p-4"><button type="button" onClick={() => setShowUtilityAdvanced((current) => !current)} className="w-full text-left"><span className="text-sm font-semibold">{showUtilityAdvanced ? "▾ Ocultar cálculo avanzado" : "▸ Tengo los datos de reparto de mi empresa"}</span><span className="mt-1 block text-xs text-[var(--muted)]">Solo abre esto si tu empresa te entregó esos datos.</span></button>{showUtilityAdvanced && <div className="mt-5"><div className="grid gap-4 sm:grid-cols-2"><Field label="Utilidad líquida de la empresa ($)" value={profit} onChange={setProfit} help="Debe ser el valor que corresponde al reparto, no tu sueldo." /><Field label="Total de días de todas las personas" value={totalWorkerDays} onChange={setTotalWorkerDays} min={0} step="1" help="Solo si la empresa te entregó este dato." /><Field label="Número de cargas familiares" value={familyCount} onChange={setFamilyCount} min={0} max={20} step="1" /><Field label="Factor B total" value={totalFamilyFactor} onChange={setTotalFamilyFactor} min={0} step="1" help="Solo si la empresa te entregó el total de factores." /></div><div className="mt-4 grid gap-3 sm:grid-cols-3"><Result label="10% por tiempo" value={utilityReady ? money.format(values.utilities.tenPercent) : "No disponible"} /><Result label="5% por cargas" value={utilityReady ? money.format(values.utilities.fivePercent) : "No disponible"} /><Result label="Total estimado" value={utilityReady ? money.format(values.utilities.total) : "Completa los datos"} /></div></div>}</div>
            </section>
          </div>
