...
export default function LaboralPage() {
  ...
  const values = useMemo(() => {
    ...
    const reserveAverage = reserveCount > 0
      ? monthlyValues.slice(0, reserveCount).reduce((sum, value) => sum + value, 0) / reserveCount
      : 0;
        ...
        <section className="py-10 sm:py-14">
          ...
          <div className="mt-8 rounded-2xl border border-[var(--border)] bg-white p-5 sm:p-6">
            ...
            <div className="mt-5 grid gap-5 sm:grid-cols-2">
              ...
              <label className="block"><span className="text-sm font-medium">Región</span><select value={region} onChange={(e) => setRegion(e.target.value as LaborRegion)} className="mt-2 min-h-11 w-full rounded-xl border border-[var(--border)] bg-white px-3 text-sm"><option value="costa">Costa / Insular</option><option value="sierra">Sierra / Amazonía</option></select><span className="mt-1 block text-xs leading-5 text-[var(--muted)]">Solo afecta el período de pago del décimo cuarto.</span></label>
            </div>
            <div className="mt-4 rounded-xl border border-[var(--border)] bg-[var(--background)] p-4 text-sm leading-6 text-[var(--muted)]"><strong className="text-[var(--foreground)]">Ejemplo:</strong> si tu sueldo base es $482 pero un mes recibiste $700 por horas extra, deja $482 arriba y registra $700 en ese mes dentro del décimo tercero. Así la calculadora no pierde tus ingresos variables.</div>
          </div>
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-2">
            <section className="rounded-2xl border border-[var(--border)] bg-white p-5 lg:col-span-2">
              ...
              <p className="mt-1 text-sm leading-6 text-[var(--muted)]">Se calcula con las remuneraciones percibidas durante el período. La forma más precisa es registrar lo recibido en cada mes; el Ministerio del Trabajo utiliza este mismo esquema en su calculadora. citeturn1search11</p>
            ...
            <section className="rounded-2xl border border-[var(--border)] bg-white p-5">
              ...
              <p className="mt-1 text-sm leading-6 text-[var(--muted)]">No depende de tu sueldo ni de tus horas extra. Se calcula con el SBU y el tiempo trabajado del período correspondiente. citeturn1search6</p>
              ...
              <p className="mt-3 text-xs text-[var(--muted)]">SBU 2026: {money.format(SBU_2026)}. citeturn2search0turn2search17</p>
            </section>

            <section className="rounded-2xl border border-[var(--border)] bg-white p-5">
              <h2 className="text-xl font-semibold">🏖️ Vacaciones</h2>
              <p className="mt-1 text-sm leading-6 text-[var(--muted)]">Aquí sí importa lo que realmente ganaste durante el año. La ley toma la veinticuatroava parte de lo percibido, incluyendo horas ordinarias, suplementarias y extraordinarias que correspondan. citeturn1search17</p>
              ...
              <div className="mt-4 rounded-xl border border-[var(--border)] bg-[var(--background)] p-3 text-xs leading-5 text-[var(--muted)]">Este valor no significa que te paguen las vacaciones como un bono aparte cada año. Es una referencia del valor de la remuneración correspondiente al período vacacional o a su parte proporcional cuando corresponda.</div>
            </section>

            <section className="rounded-2xl border border-[var(--border)] bg-white p-5">
              <h2 className="text-xl font-semibold">🏦 Fondo de reserva</h2>
              <p className="mt-1 text-sm leading-6 text-[var(--muted)]">Después del primer año de servicio, corresponde una remuneración mensual por cada año completo. Si tus ingresos cambian por horas extra, puedes usar los valores mensuales para obtener una referencia más realista. citeturn1search18</p>
            ...
            <section className="rounded-2xl border border-[var(--border)] bg-white p-5">
              ...
              <p className="mt-1 text-sm text-[var(--muted)]">Base horaria: sueldo mensual ÷ 240. Las horas suplementarias llevan 50% y las extraordinarias 100% de recargo, según corresponda. citeturn1search10</p>
            ...
            <section className="rounded-2xl border border-[var(--border)] bg-white p-5 lg:col-span-2">
              ...
              <p className="mt-1 text-sm leading-6 text-[var(--muted)]">El 15% legal se reparte en 10% por tiempo trabajado y 5% por cargas familiares. Lo importante: para calcularlo correctamente necesitas algunos datos que normalmente vienen de la empresa. citeturn3search0</p>
              ...
              <div className="mt-5 rounded-xl border border-[var(--border)] bg-[var(--background)] p-4">
                ...
                <p className="mt-1 text-xs leading-5 text-[var(--muted)]">Puedes pensar en meses si no recuerdas los días exactos. La calculadora los convierte a 30 días por mes para esta estimación.</p>
              ...
              <div className="mt-5 rounded-xl border border-[var(--border)] p-4">
                ...
                <div className="mt-4 rounded-xl bg-[var(--background)] p-3 text-xs leading-5 text-[var(--muted)]"><strong className="text-[var(--foreground)]">¿Por qué necesito ese total?</strong> Porque el 10% no se divide simplemente entre trabajadores: se reparte según los días trabajados de cada persona frente a la suma de días de todos.</div>
              </div>

              <div className="mt-5 rounded-xl border border-[var(--border)] p-4">
                <p className="text-sm font-semibold">👨‍👩‍👧 Cargas familiares · 5%</p>
                <div className="mt-4 grid gap-4 sm:grid-cols-2"><Field label="¿Cuántas cargas familiares tienes?" value={familyCount} onChange={setFamilyCount} min={0} max={20} step="1" help="Cuenta solo las cargas familiares que estén debidamente acreditadas para el reparto." /><Field label="Factor B total de la empresa" value={totalFamilyFactor} onChange={setTotalFamilyFactor} min={0} step="1" help="Es la suma de los factores A de todas las personas participantes. Si no lo tienes, no se puede calcular exactamente el 5%." /></div>
                ...
                <div className="mt-3 rounded-xl bg-[var(--background)] p-3 text-xs leading-5 text-[var(--muted)]">El Factor A se obtiene multiplicando tus días trabajados por tus cargas. El Factor B es la suma de los Factores A de todos los trabajadores y extrabajadores. citeturn3search0turn3search1</div>
              </div>
