...
const thirteenthMonths = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];
...
export default function LaboralPage() {
  ...
  const [reserveMode, setReserveMode] = useState<"base" | "monthly">("base");
        ...
        <section className="py-10 sm:py-14">
          ...
          <div className="mt-6 grid gap-5 lg:grid-cols-2">
            ...
            <section className="rounded-2xl border border-[var(--border)] bg-white p-5">
              ...
              <p className="mt-1 text-sm leading-6 text-[var(--muted)]">Primero puedes usar la estimación rápida con tu sueldo base. Si tus ingresos remunerativos cambian, abre el registro mensual para usar el promedio de los meses.</p>
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <button type="button" onClick={() => setReserveMode("base")} className={`min-h-12 rounded-xl border px-3 text-left ${reserveMode === "base" ? "border-[var(--wine)] bg-[var(--wine)]/5" : "border-[var(--border)]"}`}><span className="block text-sm font-semibold">🧾 Estimación rápida · sueldo base</span><span className="block text-xs text-[var(--muted)]">Calcula sin abrir el panel de meses.</span></button>
                <button type="button" onClick={() => setReserveMode("monthly")} className={`min-h-12 rounded-xl border px-3 text-left ${reserveMode === "monthly" ? "border-[var(--wine)] bg-[var(--wine)]/5" : "border-[var(--border)]"}`}><span className="block text-sm font-semibold">📊 Usar ingresos mensuales</span><span className="block text-xs text-[var(--muted)]">Abre el panel de enero a diciembre.</span></button>
              </div>
              ...
              {reserveMode === "monthly" && <div className="mt-4 rounded-xl border border-[var(--border)] p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold">Ingresos de enero a diciembre</p><p className="mt-1 text-xs text-[var(--muted)]">Puedes registrar los valores mensuales para calcular una referencia con tus ingresos reales.</p></div><div className="flex gap-2"><button type="button" onClick={fillBaseSalary} className="rounded-lg border border-[var(--border)] px-3 py-2 text-xs font-medium">Llenar con sueldo base</button><button type="button" onClick={clearMonthlyRemunerations} className="rounded-lg border border-[var(--border)] px-3 py-2 text-xs font-medium">Limpiar</button></div></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{monthlyRemunerations.map((value, index) => <Field key={index} label={thirteenthMonths[index]} value={value} onChange={(next) => updateMonth(index, next)} min={0} step="0.01" placeholder="482" />)}</div></div>}
