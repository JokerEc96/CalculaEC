...
const units: Record<Category, Unit[]> = {
  ...
  volumen: [
    ...
    { id: "ml", label: "Mililitros", symbol: "mL", factor: 0.001 },
    { id: "gal_us", label: "Galones (EE. UU.)", symbol: "gal", factor: 3.785411784 },
    { id: "qt", label: "Cuartos (EE. UU.)", symbol: "qt", factor: 0.946352946 },
    { id: "cup", label: "Tazas (EE. UU.)", symbol: "cup", factor: 0.2365882365 },
  ],
  temperatura: [
  ...
  velocidad: [
    { id: "kmh", label: "Kilómetros por hora", symbol: "km/h", factor: 1 },
    { id: "mph", label: "Millas por hora", symbol: "mph", factor: 1.609344 },
    { id: "ms", label: "Metros por segundo", symbol: "m/s", factor: 3.6 },
    { id: "kn", label: "Nudos", symbol: "kn", factor: 1.852 },
  ],
  area: [
    { id: "m2", label: "Metros cuadrados", symbol: "m²", factor: 1 },
    { id: "km2", label: "Kilómetros cuadrados", symbol: "km²", factor: 1_000_000 },
    { id: "ha", label: "Hectáreas", symbol: "ha", factor: 10_000 },
    { id: "ft2", label: "Pies cuadrados", symbol: "ft²", factor: 0.09290304 },
    { id: "acre", label: "Acres", symbol: "acre", factor: 4046.8564224 },
  ],
  tiempo: [
    { id: "s", label: "Segundos", symbol: "s", factor: 1 },
    { id: "min", label: "Minutos", symbol: "min", factor: 60 },
    { id: "h", label: "Horas", symbol: "h", factor: 3600 },
    { id: "day", label: "Días", symbol: "d", factor: 86400 },
    { id: "week", label: "Semanas", symbol: "sem", factor: 604800 },
  ],
  datos: [
    { id: "b", label: "Bytes", symbol: "B", factor: 1 },
    { id: "kb", label: "Kilobytes", symbol: "KB", factor: 1024 },
    { id: "mb", label: "Megabytes", symbol: "MB", factor: 1024 ** 2 },
    { id: "gb", label: "Gigabytes", symbol: "GB", factor: 1024 ** 3 },
    { id: "tb", label: "Terabytes", symbol: "TB", factor: 1024 ** 4 },
  ],
};

const format = (value: number) =>
  new Intl.NumberFormat("es-EC", { maximumFractionDigits: 8 }).format(Number.isFinite(value) ? value : 0);
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {categories.map((item) => (
              <button key={item.id} type="button" onClick={() => changeCategory(item.id)} className={`rounded-2xl border p-4 text-left transition hover:-translate-y-0.5 ${category === item.id ? "border-[var(--wine)]/25 bg-[var(--wine)] text-white shadow-md" : "border-[var(--border)] bg-white"}`}>
                ...
                <p className={`mt-1 text-xs leading-5 ${category === item.id ? "text-white/70" : "text-[var(--muted)]"}`}>{item.description}</p>
              </button>
            ))}
          </div>

          <section className="mt-5 rounded-[2rem] border border-[var(--border)] bg-white p-6 shadow-[0_18px_60px_rgba(37,35,33,0.06)] sm:p-8" aria-label="Conversor">
            ...
            <div className="mt-7 grid gap-4 lg:grid-cols-[1fr_auto_1fr] lg:items-end">
              <label className="block">
                <span className="text-sm font-semibold">Tengo</span>
                <input type="number" inputMode="decimal" value={value} onChange={(e) => setValue(e.target.value)} className="mt-2 w-full rounded-2xl border border-[var(--border)] bg-[var(--background)] px-4 py-4 text-2xl outline-none focus:border-[var(--wine)] focus:ring-4 focus:ring-[var(--wine)]/8" />
                <select value={from} onChange={(e) => setFrom(e.target.value)} className="mt-2 w-full rounded-2xl border border-[var(--border)] bg-white px-4 py-3.5 text-sm outline-none focus:border-[var(--wine)]">
                  {currentUnits.map((unit) => <option key={unit.id} value={unit.id}>{unit.label} ({unit.symbol})</option>)}
                </select>
              </label>
              ...
              <div>
                ...
                <div className="mt-2 rounded-2xl bg-[var(--wine)] p-4 text-white">
                  <p className="text-3xl font-semibold tracking-tight">{format(result)} {toUnit?.symbol}</p>
                  <p className="mt-1 text-xs text-white/70">{fromUnit?.label} → {toUnit?.label}</p>
                </div>
                <select value={to} onChange={(e) => setTo(e.target.value)} className="mt-2 w-full rounded-2xl border border-[var(--border)] bg-white px-4 py-3.5 text-sm outline-none focus:border-[var(--wine)]">
                  {currentUnits.map((unit) => <option key={unit.id} value={unit.id}>{unit.label} ({unit.symbol})</option>)}
                </select>
              </div>
            </div>

            <div className="mt-8 rounded-2xl bg-[var(--background)] p-5 text-sm text-[var(--muted)]">
              <strong className="text-[var(--foreground)]">Conversión instantánea:</strong> no necesitas presionar un botón. Escribe el valor y cambia las unidades para ver el resultado inmediatamente.
            </div>
          </section>
        </section>
      </div>
