...
export default function TripCalculatorPage() {
  ...
  async function geocodeLocation(query: string): Promise<GeocodeResult> {
    ...
    if (!Array.isArray(data) || data.length === 0) throw new Error(`No encontramos resultados para "${query}".`);
    const result = data[0];
    if (!result || typeof result.lat !== "number" || !Number.isFinite(result.lat) || typeof result.lng !== "number" || !Number.isFinite(result.lng)) {
  ...
  async function handleCalculate(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const [originLocation, destinationLocation] = await Promise.all([
        originCoordinate ? Promise.resolve({ lat: originCoordinate[0], lng: originCoordinate[1] }) : geocodeLocation(originQuery),
        destinationCoordinate ? Promise.resolve({ lat: destinationCoordinate[0], lng: destinationCoordinate[1] }) : geocodeLocation(destinationQuery),
      ]);
      if (originLocation.lat === destinationLocation.lat && originLocation.lng === destinationLocation.lng) {
  ...
  function openPicker(target: "origin" | "destination") { setError(null); setPickerTarget(target); }
